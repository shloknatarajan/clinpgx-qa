...
def analyze_eval_results(file_paths):
    ...
    chained_results = defaultdict(
        lambda: {
            "total": 0,
            "all_correct": 0,
            "turn_correct": defaultdict(lambda: {"total": 0, "correct": 0}),
        }
    )

    for file_path in file_paths:
        model_name = file_path.split(os.sep)[-2].split("_")[
            -1
            ...
            for line in f:
                ...
                if "yes_no_eval_results" in file_path:
                    ...
                    for turn in record["turns"]:
                        ...
                        chained_results[model_name]["turn_correct"][reasoning_type][
                            "total"
                        ] += 1
                        if turn["correct"]:
                            chained_results[model_name]["turn_correct"][reasoning_type][
    ...
    for model, metrics in yes_no_results.items():
        ...
        parsed_rate = (
            (metrics["parsed"] / metrics["total"]) * 100 if metrics["total"] > 0 else 0
        )
    ...
    for model, metrics in chained_results.items():
        all_correct_rate = (
            (metrics["all_correct"] / metrics["total"]) * 100
            if metrics["total"] > 0
            else 0
        )
        ...
        for turn_type, turn_metrics in metrics["turn_correct"].items():
            turn_accuracy = (
                (turn_metrics["correct"] / turn_metrics["total"]) * 100
                if turn_metrics["total"] > 0
                else 0
            )
            chained_summary.append(
                {
