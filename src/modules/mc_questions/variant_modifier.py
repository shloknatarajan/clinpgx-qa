...
Usage:
    python -m src.modules.mc_questions.variant_modifier
    python -m src.modules.mc_questions.variant_modifier --seed 42 --output data/mcq_options/variant_mcq_options.jsonl
"""
...
def generate_all_variant_mcqs(
    ...
    logger.info(f"Generated {total} MCQ entries")
    if notes_counter:
        logger.info("Generation notes distribution:")
        for key, count in sorted(notes_counter.items(), key=lambda x: -x[1]):
            logger.info(f"  {key}: {count}")

    write_simplified_mcqs(tmp_path, answer_key="variant", start_id=1, output_path=out)
    tmp_path.unlink()
    return out
